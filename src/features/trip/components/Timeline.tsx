...
export default function Timeline({ events, dayNumber = 1, isAdmin }: TimelineProps) {
  ...
  return (
    <div className="relative bg-stone-50 px-0 md:px-3 pb-20 pt-8">
      {/* Vertical connecting line - move slightly left */}
      <div
        className={`absolute left-[7px] md:left-[23px] top-0 h-full w-px ${theme.line}`}
      />

      <div className="relative space-y-6">
        {events.map((event, index) => (
          <div key={index} className="relative flex gap-1 md:gap-4">
            {/* Left column: dot */}
            <div className="relative flex w-4 md:w-6 shrink-0 flex-col items-center pt-3">
      ...
      {/* End marker */}
      <div className={`absolute bottom-8 left-0 md:left-[17px] h-4 w-4 rounded-full border-4 border-white shadow-sm ${theme.bg} opacity-30`} />
    </div>
  );
}
