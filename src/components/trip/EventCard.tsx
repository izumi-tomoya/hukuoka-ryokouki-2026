...
import YataiLiveTracker from '@/components/trip/client/YataiLiveTracker';
import TransitTimeline from '@/components/trip/TransitTimeline';
...
function BasicCard({ event, isAdmin }: { event: TripEvent, isAdmin?: boolean }) {
  ...
  return (
    <MagazineCard className={cn("h-full", event.isConfirmed && 'opacity-60 grayscale-[0.5]')}>
      ...
      <h3 className="font-playfair text-2xl font-bold text-stone-900 tracking-tight leading-snug mb-3">
        {event.title}
      </h3>
      ...
      {event.transitSteps && event.transitSteps.length > 0 && (
        <div className="mb-8">
          <TransitTimeline steps={event.transitSteps} />
        </div>
      )}
      ...
      {hasMemoir && (
        <div className="mt-8 border-t border-stone-100 pt-8 animate-in fade-in duration-700">
          ...
          {event.notes && (
            <p className="font-playfair text-lg italic text-stone-600 leading-relaxed mb-6">
